 ...
 // msg.add(px);
  ...
  void oscEvent(OscMessage theOscMessage) {
    if (theOscMessage.checkAddrPattern("/wek/outputs")) {
      int klass = (int)theOscMessage.get(0).floatValue(); 
      println(klass);
      if (klass == 1) tone=TONE_C;
      if (klass == 2) tone=TONE_g;
      if (klass == 3) tone=TONE_c;
      if (klass == 4) tone=TONE_d;
      if (klass == 5) tone=TONE_e;
